...
def check(dir_path):
  ...
  for host in HOSTS:
    text = f'- {host}: '
    parsed_url = duplicity.backend.ParsedUrl('file://' +
                                             os.path.join(dir_path, host))
    ...
    col_stats = duplicity.dup_collections.CollectionsStatus(
        backend, duplicity.config.archive_dir_path).set_values()
    ...
    if not chain:
      ...
      if datetime.datetime.fromtimestamp(
          fullset.get_time(),
          tz=datetime.UTC) < now - datetime.timedelta(days=37):
      ...
      if lastset is not fullset:
        ...
        if datetime.datetime.fromtimestamp(
            lastset.get_time(),
            tz=datetime.UTC) < now - datetime.timedelta(days=3):
...
def check_wifi(dir_path):
  ...
  def _set_str(label, regexp, scale):
    files = [
        f for f in os.listdir(f'{dir_path}/wifi.lab/') if re.match(regexp, f)
    ]
  ...
  text += ', '
  text += _set_str('Sysupgrade Full',
                   re.compile(r'sysupgrade-full.(.*).tar.gz.p7m'), 'KB')
  print_tee(text)
...
def check_cinderblock(dir_path):
  ...
  def _set_str(label, regexp, scale, max_age):
    files = [
        f for f in os.listdir(f'{dir_path}/cinderblock.lab/')
  ...
  text += _set_str('ZFS Full',
                   re.compile(r'zroot-full.(.*).zfs.p7m'),
                   'GB',
                   max_age=datetime.timedelta(days=37))
  text += ', '
  text += _set_str('ZFS Incremental',
                   re.compile(r'zroot-inc.(.*).zfs.p7m'),
                   'GB',
                   max_age=datetime.timedelta(days=7))
  text += ', '
  text += _set_str('Conf Full',
                   re.compile(r'conf-full.(.*).tar.p7m'),
                   'MB',
                   max_age=datetime.timedelta(days=7))
  print_tee(text)
...
def main():
  ...
  if args.e:
    ...
    subprocess.run(['/sbin/sendmail', '-t', '-oi'],
                   input=m.as_string(),
                   encoding='ascii',
                   check=True)
  ...
  if args.m:
    mattermost = mattermostdriver.Driver({
        ...
        'login_id':
            'bahamut',
        'token':
            open('/usr/local/etc/foulab-deaddrop-mattermost-token'
                ).read().strip(),
    })
    mattermost.login()
